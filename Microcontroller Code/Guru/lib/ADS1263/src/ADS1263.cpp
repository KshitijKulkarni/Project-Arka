...
ADS1263::ADS1263(uint8_t InitCSSelectPin, SPIClass* InitSPI)
{
	CSSelectPin = InitCSSelectPin;
	SPIBus = InitSPI;
	pinMode(CSSelectPin, OUTPUT);
...
ADS1263::ADS1263(uint8_t InitCSSelectPin, uint8_t InitDataReadyPin, SPIClass* InitSPI) : ADS1263(InitCSSelectPin, InitSPI)
{
	DataReadyPin = InitDataReadyPin;
	UseDataReadyPin = true;
	pinMode(DataReadyPin, INPUT);
}
ADS1263::ADS1263(uint8_t InitCSSelectPin, uint8_t InitDataReadyPin, uint8_t InitStartPin, SPIClass* InitSPI) : ADS1263(InitCSSelectPin, InitDataReadyPin, InitSPI)
{
	StartPin = InitStartPin;
	UseStartPin = true;
	pinMode(StartPin, OUTPUT);
...
bool ADS1263::SendRecieve(OpCodesSimple OpCodeToSend, uint8_t* ReceivedData, uint8_t BytesToRecieve)
{
	...
	{
		ReceivedData[Index] = SPIBus->transfer(0);
	}
	digitalWrite(CSSelectPin, HIGH);
	SPIBus->endTransaction();
	if (Verbose)
...
bool ADS1263::ReadRegister(uint8_t RegisterAddress, uint8_t* ReceivedData, uint8_t BytesToRecieve)
{
	if (BytesToRecieve > 0)
	{
		...
		SPIBus->beginTransaction(ConnectionSettings);
		digitalWrite(CSSelectPin,LOW);
		SPIBus->transfer(OpCodeToSend);
		SPIBus->transfer(ByteCountToSend);
		for (uint8_t Index = 0; Index < BytesToRecieve; ++Index)
		{
			ReceivedData[Index] = SPIBus->transfer(0);
		}
...
bool ADS1263::WriteRegister(uint8_t RegisterAddress, uint8_t* WriteData, uint8_t BytesToSend)
{
	if (BytesToSend > 0)
	{
		...
		for (uint8_t Index = 0; Index < BytesToSend; ++Index)
		{
			SPIBus->transfer(WriteData[Index]);
		}
		digitalWrite(CSSelectPin, HIGH);
		SPIBus->endTransaction();
		if (Verbose)
